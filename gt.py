...
MODEL = 'google/gemma-3-4b-it'  # Example model, adjust as needed
...
def load_game_config(game_name):
    ...
    try:
        ...
        print("⚠️ config/config.json not found. Using defaults.")
        return {}
...
def get_filled_context(game_name):
    ...
    if game_name == 'salop':
        ...
        # --- CALCULATE MISSING PRICES FOR CONTEXT ---
        # Base Demand (120) - (N(3) * Cournot Q(25) * Slope(1)) = 120 - 75 = 45
        expected_price_war_price = 45 
        # Base Demand (120) - (N(3) * Collusive Q(17) * Slope(1)) = 120 - 51 = 69
        expected_collusive_price = 69
        ...
        context.update({
            ...
            # --- ADDED VARIABLES ---
            "expected_price_war_price": expected_price_war_price,
            "expected_collusive_price": expected_collusive_price
        })
...
print(f"🚀 Starting reasoning tests for: {', '.join(games)}\n")

for game in games:
    print(f"{'='*40}")
    print(f"🎮 Generating Reasoning for: {game.upper()}")
    print(f"{'='*40}")
    
    # 1. Read Prompt Template
    prompt_path = Path(f'prompts/{game}.md')
    if not prompt_path.exists():
        print(f"❌ Template not found at {prompt_path}")
        continue
...
try:
    ...
    print(f"💾 All reasoning results saved to: {OUTPUT_FILE}")
except Exception as e:
